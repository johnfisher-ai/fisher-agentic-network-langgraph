...
def build():
  ...
  <section>
    ...
    <p>Building this surfaced a defect worth repeating, because it is easy to reproduce and it
    defeats the entire purpose of an approval gate. In the original, approval routed back
    through the broker, which re-ran synthesis. The answer the user received was therefore a
    second generation, sometimes carrying a different risk verdict than the one they had just
    been shown and approved. In one recorded run the human approved a proposal marked
    <code>RISK: YES</code> and the system returned an answer marked <code>RISK: NO</code>.</p>

    <p>The gate is now terminal: it returns the approved text verbatim, and a test asserts that
    the proposal appears unchanged in the final answer. If you build one of these, put that
    assertion in early.</p>
  </section>

  <section>
    <h2>Read on</h2>
    <div class="cards">
      <div class="card"><h3>The architecture</h3><p>The state machine, how the roster becomes
      nodes at load time, and how the output channel keeps the graph independent of its front
      end.</p></div>
      <div class="card"><h3>The code</h3><p>Python and XML side by side, showing one graph
      serving two domains.</p></div>
      <div class="card"><h3>Running it</h3><p>Install, key setup, what it costs, and a Colab
      notebook you can run yourself.</p></div>
    </div>
